...
public class PlayerControllerComplete : NetworkBehaviour
{
    ...
    public NetworkVariable<float> networkDashCooldown = new NetworkVariable<float>(
    ...
    public override void OnNetworkSpawn()
    {
        ...
        if (IsServer)
        {
            if (SaveSystem.instance != null && SaveSystem.instance.pendingLoad != null)
            {
                ...
                //Ignora el archivo de guardado si la vida es 0 o negativa
        ...
        ActualizarUIVida(networkHealth.Value);
    }
    ...
    {
        ActualizarUIVida(newVal);
    }

    private void ActualizarUIVida(int currentHealthValue)
    {
        if (healthImage != null)
        {
            healthImage.fillAmount = Mathf.Clamp01((float)currentHealthValue / maxHealth);
        }
    }
    ...
    { 
        ...
        { 
            CameraFollow scriptCamara = camaraPrincipal.GetComponent<CameraFollow>(); 

            if (scriptCamara != null) 
            {
                scriptCamara.target = this.transform; 
            }
        } 
    }

    private void VincularMinimapa() 
    { 
        MinimapController minimap = FindAnyObjectByType<MinimapController>(); 

        if (minimap != null) 
        {
            minimap.playerTarget = this.transform; 
        }
    }

    private void VincularUI()
    {
        if (healthImage == null) 
        { 
            ...
            if (objVida != null) 
            {
                healthImage = objVida.GetComponent<Image>(); 
            }
        }
        ...
        { 
            ...
            if (objDash != null) 
            {
                dashCooldownImage = objDash.GetComponent<Image>(); 
            }
        }
    }
    ...
    void Update()
    {
        ...
        if (IsOwner)
        {
            float dashPercentage = 0f;

            if (currentSprintCharges < maxSprintCharges) 
            {
                dashPercentage = sprintCooldownTimer / sprintCooldown;
            }

            if (dashCooldownImage != null) 
            {
                dashCooldownImage.fillAmount = dashPercentage;
            }

            networkDashCooldown.Value = dashPercentage;
        }
    ...
    void FixedUpdate()
    {
        ...
        if (isDead || (GameManager.instance != null && GameManager.instance.isPaused) || isTalking || !IsOwner) 
        {
            return;
        }
    ...
    { 
        ...
        {
            fxController.PlayJumpDust(); 
        }
    }

    private void ApplyJumpHold() 
    { 
        if (!isJumping || !jumpHeld || jumpHoldTimer >= jumpHoldMaxTime) 
        {
            return; 
        }
    ...
    private void Attack()
    {
        ...
        if (fxController != null) 
        {
            fxController.PlayAttackSlash(isFacingRight);
        }
    ...
    { 
        if (transform.position.y < voidYThreshold && !isDead) 
        {
            RequestTakeDamageServerRpc(999); 
        }
    }
    ...
    [ServerRpc]
    public void ReachGoalServerRpc(float finalTime)
    {
        if (RoundManager.instance != null)
        {
            RoundManager.instance.FinalizarRonda(finalTime);
        }
    }

    [ClientRpc]
    public void MostrarResultadosFinalesClientRpc(float t1, float t2)
    {
        Time.timeScale = 0f;

        if (RoundManager.instance != null)
        {
            if (RoundManager.instance.panelResultados != null)
            {
                RoundManager.instance.panelResultados.SetActive(true);
            }

            string ganador;
            if (t1 < t2)
            {
                ganador = "¡Ganó el Primer Plataformero!";
            }
            else
            {
                ganador = "¡Ganó el Segundo Plataformero!";
            }

            if (RoundManager.instance.textoResultados != null)
            {
                RoundManager.instance.textoResultados.text = $"Tiempo Ronda 1: {t1:F2}s\nTiempo Ronda 2: {t2:F2}s\n\n{ganador}";
            }
        }
    }
